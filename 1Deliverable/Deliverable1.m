...
h_TX_1s = h_TX_wn * safety/h_TX_wc;

% check if max possible output for filter is larger than 1
if (h_TX_1s * worse_case_TX) > 1
    fprintf('Error! Max possible output for scaled theoretical SRRC TX, h_TX_1s is: %1.17f',(h_TX_1s * worse_case_TX));
    return;
end

% compute FR of scaled RCV filter's FR
H_TX_1s = freqz(h_TX_1s,1,w);
